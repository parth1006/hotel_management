...
def update():
   try:
     ...
     cursor = conn.cursor()
     cursor.execute("update functions set funtion_type='"+ftype.get()+"',client_name='"+cname.get()+"',booking_date='"+bdate.get()+"',check_in_date='"+idate.get()+"',check_out_date='"+odate.get()+"',room_booked="+str(rbook.get())+",halls_booked='"+hbook.get()+"',price="+str(price.get())+" where booking_id='"+bid.get()+"'")
...
def search():
    try:
        ...
        if(len(ls.index)>0):
           ...
           cname.set(ls.iloc[0][2])
           bdate.set(ls.iloc[0][3])
...
update = tk.Button(win,font=('arial',12), text = "update", command= update)
update.grid(column=3, row=30)
